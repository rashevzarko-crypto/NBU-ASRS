...
app = modal.App("asrs-zero-shot-taxonomy")
...
class ZeroShotClassifier:
    ...
    def build_messages(self, narrative: str, categories: list[str]) -> list[dict]:
        """Build chat messages for zero-shot classification with taxonomy prompt."""
...
def format_summary(metrics_rows, categories, n_train, n_test):
    ...
    lines = [
        "Zero-Shot LLM: Qwen3-8B (vLLM, taxonomy-enriched prompt)",
        "=" * 60,
...
def print_comparison(taxonomy_rows, classic_ml_path):
    """Print side-by-side F1 comparison between classic ML, basic zero-shot, and taxonomy zero-shot."""
    import pandas as pd

    classic = pd.read_csv(classic_ml_path)
    classic_f1 = dict(zip(classic["Category"], classic["F1"]))

    basic_path = "results/zero_shot_metrics.csv"
    basic_f1 = {}
    ...
    print("\n" + "=" * 85)
    print(f"{'Category':<35} {'Classic ML':>12} {'ZS Basic':>12} {'ZS Taxonomy':>13} {'Delta':>9}")
    print("-" * 85)
...
def main(test_csv: str = "data/test_set.csv", batch_size: int = BATCH_SIZE):
    ...
    checkpoint_path = "results/zero_shot_taxonomy_checkpoint.csv"
    completed = {}
    ...
    metrics_path = "results/zero_shot_taxonomy_metrics.csv"
    ...
    raw_path = "results/zero_shot_taxonomy_raw_outputs.csv"
    raw_df.to_csv(raw_path, index=False)
    print(f"Saved {raw_path}")

    # --- Save summary ---
    summary = format_summary(metrics_rows, categories, 31850, n_test)
    summary_path = "results/zero_shot_taxonomy_summary.txt"
