"""Mistral Large 3 few-shot classification of ASRS reports with taxonomy-enriched prompt.

Uses Mistral Batch API — builds all 8,044 requests as inline batch, submits once,
polls for completion, downloads results. No rate limit issues, 50% cheaper than real-time.

Usage:
    python scripts/mistral_large_few_shot.py submit    # Build & submit batch job
    python scripts/mistral_large_few_shot.py poll       # Check job status
    python scripts/mistral_large_few_shot.py results    # Download results & compute metrics
    python scripts/mistral_large_few_shot.py            # Auto: submit if no job, else poll/results
"""
...
TEMPERATURE = 0.0
MAX_TOKENS = 256
N_EXAMPLES_PER_CAT = 2  # 2 examples per category = 26 total

RESULTS_DIR = "results"
JOB_ID_PATH = os.path.join(RESULTS_DIR, "mistral_large_job_id.txt")

# ---------------------------------------------------------------------------
# Taxonomy-enriched system prompt
# ---------------------------------------------------------------------------
...
def select_few_shot_examples(train_csv: str, categories: list[str]) -> list[dict]:
    """Select N_EXAMPLES_PER_CAT diverse examples per category from training set."""
    ...
    for cat in categories:
        ...
        selected = cat_df.head(N_EXAMPLES_PER_CAT)
...
def build_messages(narrative: str, examples: list[dict]) -> list[dict]:
...
def submit_batch(test_csv: str, train_csv: str):
    ...
    # Build JSONL file
    jsonl_path = os.path.join(RESULTS_DIR, "mistral_large_batch.jsonl")
    print(f"\nBuilding {n_test} batch requests to {jsonl_path}...")
    with open(jsonl_path, "w", encoding="utf-8") as f:
        for acn, narrative in zip(acns, narratives):
            messages = build_messages(narrative, examples)
            line = json.dumps({
                "custom_id": str(acn),
                "body": {
    ...
    job = client.batch.jobs.create(
        ...
        metadata={"experiment": "asrs-mistral-large-few-shot"},
    )
...
def download_and_process(job_id: str = None):
    ...
    metrics_path = os.path.join(RESULTS_DIR, "mistral_large_metrics.csv")
    ...
    raw_path = os.path.join(RESULTS_DIR, "mistral_large_raw_outputs.csv")
    raw_df.to_csv(raw_path, index=False)
    print(f"Saved {raw_path}")

    # Save summary
    summary = format_summary(metrics_rows, n_test)
    summary_path = os.path.join(RESULTS_DIR, "mistral_large_summary.txt")
...
# ---------------------------------------------------------------------------
# Parsing helpers (same 3-tier approach as modal_few_shot.py)
# ---------------------------------------------------------------------------
...
def format_summary(metrics_rows, n_test):
    ...
    lines = [
        "Few-Shot LLM: Mistral Large 3 (Batch API, taxonomy-enriched prompt)",
        "=" * 70,
        f"Test set: {n_test} reports | Model: {MODEL}",
        f"Batch API: temperature={TEMPERATURE}, max_tokens={MAX_TOKENS}",
        f"Few-shot: {N_EXAMPLES_PER_CAT} examples/category, 600 char truncation, taxonomy subcategories in prompt",
        "",
...
def print_comparison(mistral_rows):
    """Print comparison: Classic ML vs Qwen3 few-shot vs Mistral Large few-shot."""
    classic_path = os.path.join(RESULTS_DIR, "classic_ml_text_metrics.csv")
    qwen_path = os.path.join(RESULTS_DIR, "few_shot_metrics.csv")
    ...
    if os.path.exists(qwen_path):
        ...
        qwen_f1 = dict(zip(qwen["Category"], qwen["F1"]))

    print("\n" + "=" * 95)
    print(f"{'Category':<35} {'Classic ML':>12} {'Qwen3 FS':>12} {'Mistral L':>12} {'vs Qwen3':>10}")
    print("-" * 95)
    for row in mistral_rows:
        cat = row["Category"]
        mf1 = row["F1"]
        cf1 = classic_f1.get(cat, float("nan"))
        qf1 = qwen_f1.get(cat, float("nan"))
        delta = mf1 - qf1 if not (qf1 != qf1) else float("nan")
        print(f"{cat:<35} {cf1:>12.4f} {qf1:>12.4f} {mf1:>12.4f} {delta:>+10.4f}")
    print("=" * 95)
...
def main():
    ...
    if action == "submit":
        submit_batch("data/test_set.csv", "data/train_set.csv")
        ...
        print("Usage: python scripts/mistral_large_few_shot.py [submit|poll|results|auto]")
        sys.exit(1)
