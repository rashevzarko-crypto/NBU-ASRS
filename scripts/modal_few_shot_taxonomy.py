...
with taxonomy-enriched system prompt (from Mistral Large experiment).
...
app = modal.App("asrs-few-shot-taxonomy")
...
def format_summary(metrics_rows, categories, n_test):
    ...
    lines = [
        "Few-Shot LLM: Qwen3-8B (vLLM, taxonomy-enriched prompt)",
        "=" * 60,
...
def print_comparison(few_shot_rows):
    """Print comparison: basic few-shot vs taxonomy few-shot."""
    import pandas as pd
    ...
    print("\n" + "=" * 85)
    print(f"{'Category':<35} {'Classic ML':>12} {'FS Basic':>12} {'FS Taxonomy':>13} {'Delta':>9}")
    print("-" * 85)
    for row in few_shot_rows:
        ...
        delta = tf1 - bf1 if not (bf1 != bf1) else float("nan")
        print(f"{cat:<35} {cf1:>12.4f} {bf1:>12.4f} {tf1:>13.4f} {delta:>+9.4f}")
    print("=" * 85)
...
def main(
    ...
    checkpoint_path = "results/few_shot_taxonomy_checkpoint.csv"
    completed = {}
    ...
    metrics_path = "results/few_shot_taxonomy_metrics.csv"
    ...
    raw_path = "results/few_shot_taxonomy_raw_outputs.csv"
    raw_df.to_csv(raw_path, index=False)
    print(f"Saved {raw_path}")

    # --- Save summary ---
    summary = format_summary(metrics_rows, categories, n_test)
    summary_path = "results/few_shot_taxonomy_summary.txt"
