...
Sends test set narratives with few-shot examples to vLLM on a Modal L4 GPU,
...
GPU = "L4"
BATCH_SIZE = 16  # smaller than zero-shot (64) — each prompt is ~4x larger
...
class FewShotClassifier:
    ...
    def load_model(self):
        ...
        self.llm = LLM(
            model=MODEL_ID,
            max_model_len=16384,  # larger context for few-shot examples
            dtype="auto",
...
def format_summary(metrics_rows, categories, n_test):
    ...
    lines = [
        "Few-Shot LLM: Qwen3-8B (vLLM)",
        "=" * 55,
        f"Test set: {n_test} reports | Model: {MODEL_ID}",
        "vLLM: dtype=auto, max_model_len=16384, temperature=0.0, max_tokens=256",
...
def print_comparison(few_shot_rows):
    """Print three-way F1 comparison: Classic ML vs Zero-Shot vs Few-Shot."""
    import pandas as pd

    classic_path = "results/classic_ml_text_metrics.csv"
    zero_shot_path = "results/zero_shot_metrics.csv"
    ...
    if os.path.exists(zero_shot_path):
        zero = pd.read_csv(zero_shot_path)
        zero_f1 = dict(zip(zero["Category"], zero["F1"]))

    print("\n" + "=" * 85)
    print(f"{'Category':<35} {'Classic ML F1':>14} {'Zero-Shot F1':>14} {'Few-Shot F1':>13} {'Delta(ZS)':>9}")
    print("-" * 85)
    for row in few_shot_rows:
        cat = row["Category"]
        ff1 = row["F1"]
        cf1 = classic_f1.get(cat, float("nan"))
        zf1 = zero_f1.get(cat, float("nan"))
        delta = ff1 - zf1 if not (zf1 != zf1) else float("nan")  # nan check
        print(f"{cat:<35} {cf1:>14.4f} {zf1:>14.4f} {ff1:>13.4f} {delta:>+9.4f}")
    print("=" * 85)
...
def main(
    ...
    """Run few-shot classification: inference on Modal, metrics locally."""
    import pandas as pd
    import numpy as np
    ...
    checkpoint_path = "results/few_shot_checkpoint.csv"
    completed = {}
    ...
    metrics_path = "results/few_shot_metrics.csv"
    ...
    raw_path = "results/few_shot_raw_outputs.csv"
    raw_df.to_csv(raw_path, index=False)
    print(f"Saved {raw_path}")

    # --- Save summary ---
    summary = format_summary(metrics_rows, categories, n_test)
    summary_path = "results/few_shot_summary.txt"
